...
class  MyTokenObtainPairSerializer(TokenObtainPairSerializer): 
    ...
    def get_token(cls, user):
        ...
        try:
            token['vendor_id'] = token.vendor_id
        except:
            token['vendor_id'] = 0
...
class UserSerializer(serializers.ModelSerializer):
    # Define a serializer for user details, which inherits from serializers.ModelSerializer
    class Meta:
        # Specify the model that this serializer is associated with
        model = api_models.User
        # Define the fields from the model that should be included in the serializer
...
class BookmarkSerilaizer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        super(BookmarkSerilaizer, self).__init__(*args, **kwargs)
        request = self.context.get('request')
